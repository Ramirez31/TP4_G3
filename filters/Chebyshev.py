...
class Chebyshev(base_filter):
    ...
    def do_approximation(self):
       ...
       self.n = int(np.ceil(np.arccosh(np.sqrt(np.power(10,self.Ap/10)-1)/self.epsilon)/np.arccosh(self.wan)))
       if self.n is 1:
           self.n=2
       for i in range(1,self.n+1):
           ...
           pole1= np.sin(alfa)*np.sinh(np.arcsinh(beta1))+1j*np.cos(alfa)*np.cosh(beta1)
           pole2= np.sin(alfa)*np.sinh(np.arcsinh(beta2))+1j*np.cos(alfa)*np.cosh(beta2)
           if np.real(pole1)<=0:
